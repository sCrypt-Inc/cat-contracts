...
describe('Test SmartContract `Vault`', () => {
    ...
    it('should pass', async () => {
        ...
        const tapleafVaultTrigger = Tap.encodeScript(
        ...
        const tapleafVaultComplete = Tap.encodeScript(
        ...
        const tapleafVaultCancel = Tap.encodeScript(
            scriptVaultCancel.toBuffer()
        )

        const [tpubkeyVault] = Tap.getPubKey(DISABLE_KEYSPEND_PUBKEY, {
            tree: [
                tapleafVaultTrigger,
                tapleafVaultComplete,
                tapleafVaultCancel,
            ],
        })
        ...
        while (true) {
            sighash = getSigHashSchnorr(
                tx1,
                Buffer.from(tapleafVaultTrigger, 'hex'),
                0
            )
        ...
        // Also sign fee input
        let hashData = btc.crypto.Hash.sha256ripemd160(
            seckey.publicKey.toBuffer()
        )
        let signatures = tx1.inputs[1].getSignatures(
            tx1,
            seckey,
            1,
            undefined,
        ...
        let witnesses = [
            ...
            Buffer.concat([
                Buffer.from('16', 'hex'),
                txFee.outputs[0].script.toBuffer(),
            ]),
            vaultAmtBuff,
            feeAmtBuff,
            Buffer.concat([
        ...
        while (true) {
            sighash = getSigHashSchnorr(
                tx2,
                Buffer.from(tapleafVaultComplete, 'hex'),
                0
            )
            e = await getE(sighash.hash)
            eBuff = e.toBuffer(32)
        ...
        signatures = tx2.inputs[1].getSignatures(
            tx2,
            seckey,
            1,
            undefined,
